...
export default class RoomActionPanel extends React.Component<IProps, IState> {
	...
	toggleRoomNameEdit(): void {
		...
		if (this.state.editRoomNameActive) {
			...
			socketManager.sendEvent(RoomEventNames.changeRoomName, {
	...
	toggleRoomPasswordEdit(): void {
		...
		if (this.state.editRoomPassActive) {
			...
			socketManager.sendEvent(RoomEventNames.changeRoomPass, {
			...
			<div className={'room-admin-actions'}>
				<div className={'room-edit-name'}>
					...
					{this.state.editRoomNameActive ? (
						<input
							value={this.state.editedRoomName}
							onChange={(event) =>
								this.setState({ editedRoomName: event.target.value })
							}
							ref={this.roomNameInputRef}
							type={'text'}
						/>
					) : (
						<span className={'password-inactive'}>
							{roomManager.getRoomName()}
						</span>
					)}
					{/* edit room name */}
					<FontAwesomeIcon
						onClick={this.toggleRoomNameEdit.bind(this)}
						icon={[
							'fad',
							this.state.editRoomNameActive ? 'floppy-disk' : 'pen-to-square',
						]}
						size="1x"
					/>
				</div>
				...
				<div className={'room-edit-password'}>
					...
					{this.state.editRoomPassActive ? (
						<input
							...
							onChange={(event) =>
								this.setState({ editedPassword: event.target.value })
							}
						...
						<span className={'password-inactive'}>
							{this.state.editedPassword.replace(/./g, '•')}
						</span>
					)}

					{/* edit room password */}
					<FontAwesomeIcon
						onClick={this.toggleRoomPasswordEdit.bind(this)}
						icon={[
							'fad',
							this.state.editRoomPassActive ? 'floppy-disk' : passwordIcon,
						]}
						size="1x"
					/>
				</div>
				...
				<div className={'room-actions-move'}>
					...
					{profileManager.isRoomMaster() && roomManager.getCurrentGameId() ? (
						<button onClick={this.returnToGameSelection.bind(this)}>
							Zur Spiele-Auswahl{' '}
							<FontAwesomeIcon icon={['fad', 'puzzle']} size="1x" />
						</button>
					) : null}
				</div>
			</div>
