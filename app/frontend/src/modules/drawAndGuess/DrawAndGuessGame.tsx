...
export default class DrawAndGuessGame
	...
	implements ModuleGameInterface
{
	...
	componentDidMount(): void {
		this.api
			.getEventApi()
			.addEventHandler(
				'passiveCanvasChanged',
				this.onCanvasChangedRemote.bind(this)
			);
		this.api
			.getEventApi()
			.addEventHandler(
				'wordToGuessChanged',
				this.onWordToGuessChanged.bind(this)
			);
		this.api
			.getEventApi()
			.addEventHandler(
				'newGuessChatMessage',
				this.onNewGuessChatMessage.bind(this)
			);
		this.api
			.getEventApi()
			.addEventHandler(
				'activePlayerChanged',
				this.onActivePlayerChanged.bind(this)
			);
		this.api
			.getEventApi()
			.addEventHandler(
				'wordSelectionOptions',
				this.onWordSelectionOptions.bind(this)
			);
		this.api
			.getEventApi()
			.addEventHandler(
				'wordToDrawChanged',
				this.onWordToDrawChanged.bind(this)
			);
		this.api
			.getEventApi()
			.addEventHandler('drawingSolution', this.onDrawingSolution.bind(this));
		this.api
			.getEventApi()
			.addEventHandler('gameStateChanged', this.onGameStateChanged.bind(this));
		this.api
			.getEventApi()
			.addEventHandler(
				'configurationChanged',
				this.onRemoteConfigChanged.bind(this)
			);
	}
	...
	onDrawingSolution(eventData: EventDataObject): void {
		...
		this.addChatMessage(`Das Wort war: ${solution}`, null, null, 'solution');
	...
	renderDrawingBoardSpace(
		...
		switch (state) {
			...
			case 'selecting':
				return (
					<WordSelection
						onSelect={this.onWordSelection.bind(this)}
	...
	render(): ReactNode {
		...
		let allowDrawing =
			isActivePlayer && this.state.currentGameState === 'drawing';
			...
			<div id={'drawAndGuess'} key={'drawAndGuess'}>
				...
				>
					<GameStateBox
						...
						wordMask={
							allowDrawing ? this.state.currentWord : this.state.currentMask
						}
					...
					<ChatBox
						...
						isGuessing={
							!allowDrawing && this.state.currentGameState === 'drawing'
						}
					/>
				</div>
			</div>
