...
export default class GameWheel extends Component<IProps, IState> {
    ...
    componentDidUpdate(prevProps: Readonly<IProps>, prevState: Readonly<IState>, snapshot?: any) {
        ...
        const activePlayerIndex = roomMembers.findIndex(user => user.getId() === this.props.activePlayerId);
        this.changeWheelDirection(360 / roomMembers.length * activePlayerIndex);
    }

    onWheelRotationIntervalTicked(): void {
        const requiredRotation = Math.abs((this.state.currentWheelRotation % 360) - (this.desiredWheelRotation % 360));
        ...
        this.setState({
            currentWheelRotation: this.state.currentWheelRotation + this.wheelRotationStep
        });
    }

    changeWheelDirection(newOffset: number, isRelative: boolean = false): void {
        this.desiredWheelRotation = (newOffset + (isRelative ? this.desiredWheelRotation : 0));
        if(!this.wheelRotationInterval) {
            this.wheelRotationInterval = setInterval(this.onWheelRotationIntervalTicked.bind(this), this.wheelRotationIntervalSpeed);
        }
    }
    ...
    render() {
        ...
        return (
            <div className={'game-wheel'}>
                ...
                >
                    <div className={'letter-container'}>
                        {this.props.letters.map((letter, index) => {
                            return <div key={index} className={`hangman-letter ${letter !== null ? 'solved' : ''}`}>
                                {letter || <>&nbsp;</> }
                            </div>
                        })}
                    </div>
                ...
                {roomMembers.map(this.renderPlayerWheelIcon.bind(this, roomMembers.length))}
    ...
    renderPlayerWheelIcon(playerCount: number, player: User, index: number): JSX.Element {
        const rotDivisions = TWO_PI/playerCount;
        const rot = rotDivisions * index + PI + (TWO_PI/360 * this.state.currentWheelRotation);
        ...
        return (
            <div className={'game-wheel-player'}
                 key={player.getId()}
                 style={{
                     left: xPos+'%',
                     top: yPos+'%',
