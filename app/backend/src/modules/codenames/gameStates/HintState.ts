...
export default class HintState extends AbstractState {
	...
	protected isPublishHintAllowed(
		userId: string,
		hint: Hint,
		board: BoardElement[],
		teams: Team[]
		...
		if (
			...
			hint.word &&
			hint.amnt &&
			!!teams.find((team) => team.spymaster === userId)?.active &&
			!hint.word.includes(' ')
		) {
			if (
				board.find((e) => e.word === hint.word) === undefined &&
				hint.amnt > 0
			) {
				return true;
			}
			...
			this.gameApi
				.getLogger()
				.warning('Invalid action due to missing property or invalid rights');
		}
		return false;
	}
}
