...
import {ColorGrid, Coordinate, GridColorOptions} from '@edelgames/types/src/modules/colorChecker/CCTypes';
...
export default class ColorGridBox extends React.Component<IProps, IState> {
	...
	renderCell(x: number, y: number): JSX.Element {
		...
		const canBeSelected = this.canFieldBeSelected({x: x, y: y}, [GridColorOptions.GREEN], [2]);
	...
	canFieldBeSelected(pos: Coordinate, allowedColors: GridColorOptions[], allowedCounts: number[]): boolean {
		const gridWidth = this.props.colorGrid.length;
		const gridHeight = this.props.colorGrid[0].length;

		const requiredFieldCount = allowedCounts.reduce((prev, curr) => prev < curr ? prev : curr) || 0;
		...
		if(targetField.color === '#fff' || allowedColors.indexOf(targetField.color) === -1) {
		...
		if (this.state.currentSelection.length > 0 && (
			!this.state.currentSelection.find((other) =>
		...
		while(fieldsToCheck.length > 0 && nearbyFields.length < requiredFieldCount) {
			const fieldToCheck = fieldsToCheck.shift();
			if(!fieldToCheck) {
				continue;
			}

			const gridFieldToCheck = this.props.colorGrid[fieldToCheck.x][fieldToCheck.y];

			if (nearbyFields.find((field) => field.x === fieldToCheck.x && field.y === fieldToCheck.y) ||
		...
		if(pos.x === 8 && pos.y == 3) {
			console.log(`${pos.x}|${pos.y} has ${nearbyFields.length} neighbours of ${requiredFieldCount}`, nearbyFields);
		}
