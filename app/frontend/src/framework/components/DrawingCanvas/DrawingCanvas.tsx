...
export default class DrawingCanvas extends React.Component<IProps, {}> {
	...
	sketchSetup(p5: p5Types, canvasParentRef: Element) {
		p5.createCanvas(this.props.canvasWidth, this.props.canvasHeight).parent(
			canvasParentRef
		);
		p5.noStroke();
		p5.smooth();
	...
	sketchMousePressed(p5: p5Types) {
		...
		if (
			!this.props.enableManualDrawing ||
			mouseX < 0 ||
			mouseY < 0 ||
			mouseX >= p5.width ||
			mouseY >= p5.height
		...
		let color =
			this.props.drawingMode === drawingModes.ERASE
				? this.props.backgroundColor
				: this.props.drawingColor;
	...
	sketchMouseDragged(p5: p5Types) {
		if (
			!this.props.enableManualDrawing ||
			this.props.drawingMode === drawingModes.FILL
		) {
			return;
		}
	...
	render(): ReactNode {
		return (
			<Sketch
				setup={this.sketchSetup.bind(this)}
