...
import {
    possibleGameStates, ScoreCellIDs, YahtzeeScoreboardType
} from "@edelgames/types/src/modules/yahtzee/YTypes";
import {EventDataObject} from "@edelgames/types/src/app/ApiTypes";
import {
    CellSelectedEventData, DiceChangedEventData,
    DiceSelectionChangedEventData, GameStateChangedEventData,
    RollRequestedEventData, ScoresChangedEventData, YahtzeeClientToServerEventNames, YahtzeeServerToClientEventNames
...
export default class YahtzeeGame extends React.Component<{}, IState> implements ModuleGameInterface {
    ...
    componentDidMount() {
        this.api.getEventApi().addEventHandler(YahtzeeServerToClientEventNames.GAME_STATE_CHANGED, this.onGameStateChanged.bind(this));
        this.api.getEventApi().addEventHandler(YahtzeeServerToClientEventNames.DICES_CHANGED, this.onDicesChanged.bind(this));
        this.api.getEventApi().addEventHandler(YahtzeeServerToClientEventNames.SCORES_CHANGED, this.onScoresChanged.bind(this));
    }
    ...
    */
    ...
    onDicesChanged(eventData: EventDataObject): void {
        let {diceValues, diceMask, remainingRolls, hasRolledDice} = eventData as DiceChangedEventData;

        this.setState({
            diceValues,
            diceMask,
            remainingRolls,
            lastRollTimeStamp: hasRolledDice ? Date.now() : this.state.lastRollTimeStamp
        });
    }
    ...
    render(): ReactNode {
        ...
        let isLocalePlayerActive = playerApi.getLocalePlayer().getId() === activePlayerId;
            ...
            <div id={"yahtzee"}>
                ...
                />
                {
                    this.state.gameState !== possibleGameStates.ENDING_SCORE ?
                        ...
                        /> :
                        <WinningScreen playerApi={this.api.getPlayerApi()}
