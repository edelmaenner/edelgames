	...
	switch (scoreType) {
		...
		case ScoreCellIDs.THREE_OF_A_KIND: {
			...
			return hasThreeOfAKind
				? diceValues.reduce((prev, eyes) => prev + eyes)
				: 0;
		}
		case ScoreCellIDs.FOUR_OF_A_KIND: {
			const hasFourOfAKind =
				diceCollection.length <= 2 &&
				diceCollection.find((entry) => entry.count >= 4);
		...
		case ScoreCellIDs.FULL_HOUSE: {
			const hasFullHouse =
				diceCollection.length === 2 &&
				diceCollection.find((entry) => entry.count === 3);
...
function isSmallStraight(diceCollection: DiceDotCollection): boolean {
	...
	for (const entry of diceCollection) {
	...
	return (
		numbers[2] &&
		numbers[3] &&
		((numbers[1] && (numbers[0] || numbers[4])) || (numbers[4] && numbers[5]))
	);
}

export function getDiceDotCollections(diveValues: number[]): DiceDotCollection {
	const diceDots: DiceDotCollection = [];
	for (const value of diveValues) {
		const entry = diceDots.find((entry) => entry.value === value);
		if (entry) {
...
export function getTotalFirstPartPoints(
