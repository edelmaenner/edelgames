...
import {ColorGrid, ColorGridCell, ColorGridColumn, Coordinate} from "@edelgames/types/src/modules/colorChecker/CCTypes";
...
export default class ColorGridBoxAnimation extends Component<IProps, IState> {
    ...
    componentDidMount() {
        if(this.props.animationSteps && this.props.animationSteps.length > 0) {
            this.animationInterval = setInterval(this.onAnimationTimerTick.bind(this), Math.min(100,this.props.animationSpeed));
            this.setState({
                playAnimation: true
            })
        }
    ...
    onAnimationTimerTick(): void {
        ...
        this.setState({
            ...
            animationCells: this.props.animationSteps.slice(0, this.state.animationFrame)
        });
    }
    ...
    renderCell(x: number, cell: ColorGridCell, y: number): JSX.Element {
        const isChecked = this.state.playAnimation ?
            this.state.animationCells.find((el) => el.x === x && el.y === y) !== undefined :
            cell.checked;
