...
type internalEventHandlerFunction = (
	eventData: internalEventDataObject | null
) => void;
...
export default class ModuleRoomApi {
	...
	public alertEvent(
		eventName: string,
		eventData: internalEventDataObject | null = null,
		skipPrefix = false
	) {
		const event = skipPrefix ? eventName : this.getGameId() + '_' + eventName;
	...
	public addEventHandler(
		...
		const event = this.getGameId() + '_' + eventName;
		if (!this.eventListeners[event]) {
	...
	public sendPlayerMessage(
		...
		const event = this.getGameId() + '_' + eventName;
		const user = this.room
			.getRoomMembers()
			.find((user) => user.getId() === playerId);
		SocketMessenger.directMessageToSocket(
			user.getSocket(),
			'ServerToClientGameMessage',
			{
				messageTypeId: event,
