...
type internalEventHandlerFunction = (
	eventData: internalEventDataObject
) => void;
...
export default class ModuleGameApi {
	 ...
	 * @internal
	 */
	public onServerToClientMessageEventNotified(
		eventData: internalEventDataObject
	) {
	...
	public alertEvent(
		...
		let event = skipPrefix
			? eventName
			: this.game.getUniqueId() + '_' + eventName;
	...
	public getUserDataById(playerId: string): User | undefined {
		return RoomManager.getRoomMembers().find(
			(member) => member.getId() === playerId
		);
	}
}
