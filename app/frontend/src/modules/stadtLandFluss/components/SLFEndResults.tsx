...
import { EndResultProps } from '../types/SLFTypes';
...
export default class SLFEndResults extends Component<EndResultProps, {}> {
	...
	private getEndResultData(): {
		...
		Object.values(this.props.points).forEach((value) =>
			Object.values(value).forEach((data) => {
				for (let [userId, points] of Object.entries(data)) {
					finalData[userId] = {
						userId: userId,
						username:
							finalData[userId]?.username ??
							roomMembers.find((u) => u.getId() === userId)?.getUsername(),
						place: 0,
		...
		return (
			<div id={'slfEndResult'}>
				<table>
					...
					<tbody>
						{this.getEndResultData().map((d) => (
							<tr key={'endResultPlayer_' + d.userId}>
								<td key={'endResultPlayer_' + d.userId + '_place'}>
									{d.place}
								</td>
								<td key={'endResultPlayer_' + d.userId + '_username'}>
									{d.username}
								</td>
								<td key={'endResultPlayer_' + d.userId + '_points'}>
									{d.points}
								</td>
							</tr>
						))}
				...
				{this.props.isRoomMaster ? (
					<button onClick={this.restartGame.bind(this)}>Nochmal spielen</button>
				) : (
					''
				)}
			</div>
