...
import { DAGChatMessage } from '@edelgames/types/src/modules/drawAndGuess/DAGTypes';
...
export default class ChatBox extends Component<IProps, {}> {
	...
	constructor(props: IProps | Readonly<IProps>) {
		super(props);
		RoomManager.getRoomMembers().forEach(
			(member) => (this.playerNames[member.getId()] = member.getUsername())
		);
	}
	...
	renderChatMessage(message: DAGChatMessage): JSX.Element {
		let sender =
			message.sender === 'system'
				? null
				: this.playerNames[message.sender] || '???';
		...
		return (
			<div className={classNames} key={message.timestamp}>
				<span>
					{sender ? `${sender}: ` : ''}
					{message.text}
				</span>
