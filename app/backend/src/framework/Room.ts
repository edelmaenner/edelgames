...
export default class Room {
	...
	public getCurrentGameId(): string | null {
		return this.currentModuleRoomApi
			? this.currentModuleRoomApi.getGameId()
			: null;
	}
	...
	public setCurrentGame(roomApi: ModuleRoomApi | null) {
		if (this.currentModuleRoomApi && roomApi === null) {
			...
			debug(
				1,
				`stopped current game ${this.currentModuleRoomApi.getGameId()} in room ${
					this.roomId
				}`
			);
		}
	...
	public onUserNotifiedGame(
		...
		if (this.currentModuleRoomApi) {
			this.currentModuleRoomApi.alertEvent(
				eventName,
				{
					senderId: userId,
					...eventData,
	...
	public sendRoomChangedBroadcast(): void {
		this.broadcastRoomMembers('roomChanged', {
			...
			currentGameId: this.currentModuleRoomApi
				? this.currentModuleRoomApi.getGameId()
				: null,
		});
