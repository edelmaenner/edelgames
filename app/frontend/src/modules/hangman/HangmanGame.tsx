	...
	implements ModuleGameInterface
{
	...
	render(): ReactNode {
		...
		const turnErrorProgress = (this.state.gameState.phase === 'guessing' || this.state.gameState.phase === 'waiting')
			? this.state.gameState.wrongChars.length / (this.state.gameState.configuration.maxWrongGuesses+1)
			: 0;
				...
				<div className={'game-data-interface'}>
...

					{
						...
						<div className={'hangman-visualisation'}>
							<HangmanSvg className={'hangman-svg'} style={{
								strokeDashoffset: (-maxDashArrayOffset + (turnErrorProgress * maxDashArrayOffset))
							}}/>
						</div>
					}
				</div>
