...
import { ConfigProps } from '../types/SLFTypes';
	...
	private updateConfig() {
		...
		if (settingsDiv) {
			...
			let categoriesString =
				settingsDiv.getElementsByClassName('categories')[0].nodeValue;
			...
			<div id={'gameConfig'}>
				...
				<div className={'settingsWrapper'}>
					<div className={'configRow'}>
						...
						<input
							key={this.props.config.rounds}
							className={'rounds'}
							type={'number'}
							placeholder={'10'}
							defaultValue={this.props.config.rounds}
					...
					<div className={'configRow'}>
						...
						<textarea
							key={this.props.config.categories.join('|')}
							className={'categories'}
							readOnly={!this.props.isRoomMaster}
							defaultValue={this.props.config.categories.join(',')}
						></textarea>
					</div>
				</div>
				<div className={'configButtons'}>
					{this.props.isRoomMaster ? (
						<button onClick={this.updateConfig.bind(this)}>Anwenden</button>
					) : (
						''
					)}
					{this.props.isRoomMaster ? (
						<button onClick={this.startGame.bind(this)}>Spiel starten</button>
					) : (
						''
					)}
				</div>
